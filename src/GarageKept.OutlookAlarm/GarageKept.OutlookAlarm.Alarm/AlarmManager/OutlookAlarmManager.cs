...
public class OutlookAlarmManager : IAlarmManager
{
    ...
    {
        return Alarms.Values.Where(a => a.IsActive).OrderBy(a=>a.Start);
    }
    ...
    private void FetchAlarms(object? signature)
    {
        ...
        var alarms = GetAlarmsFromSource(Program.AppSettings.FetchTime).ToList();
